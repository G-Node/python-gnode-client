...
with open('080707/lfp_fix080707.dat', 'r') as f:
    ...
    for i, l in enumerate(f):
        ...
        if i < 176: # create segment every line for first 176 lines
            ...
            s = gnode.Segment.objects.get(id=int(math.fmod(i, 176))

        if (i % 176) == 0: # create recording channel every 176 lines
            r = neo.core.RecordingChannel()
            r.name = 'Channel %d' % meta_channels[ i / 176 ]
            r.index = meta_channels[ i / 176 ]
            r.block = b
            b.recordingchannels.append( r )

        # creating analogsignal
        data = convert_to_timeseries( l )
        init_data = {}
        init_data['t_start'] = 0 * pq.ms
        sr = hardware.properties['LfpSamplingRate'].value
        sr_units = hardware.properties['LfpSamplingRate'].units
        init_data['sampling_rate'] = sr * getattr(pq, sr_units)
        signal = neo.core.AnalogSignal(data, **init_data)
